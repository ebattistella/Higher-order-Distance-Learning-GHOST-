            ...
            for q in range(len(clustering_list[k])):
                ...
                if cluster_metric == -1 or cluster_metric == 2:#If the cluster-metric is involved
                    aux += np.array([parallel_dist_order(graphs[k], method, clusters_idx[k][i_clust], q, 1) for
                        ...
                        np.sum([graphs[k].paths[p][q] for p in clusters_idx[k][i_clust]]) / np.sum([graphs[k].paths[p][p2]
                        for p in clusters_idx[k][i_clust] for p2 in clusters_idx[k][i_clust] if p2 > p])])
    ...
    while counter < max_it and conv_count < 3:
        ...
        for i in range(T):
            ...
            if cluster_metric == -1 or cluster_metric == 2:
                for method in range(len(methods)):
                    for q in clusters_idx[k][i_clust]:
                        w[idx_update + method + 1] -= s_t * parallel_dist_order(graphs[k], method, clusters_idx[k][i_clust], q,
                             1 if np.sum([x[k][p, q] for p in clusters_idx[k][i_clust]]) == 1 else 0)
            ...
            for k in range(K):
                ...
                if cluster_metric > 0:
                    ...
                    for q in range(len(clusters)):
                        ...
                        distance_higher[:,q,-1] += np.array([np.sum([
                            (graphs[k].paths[p][clusters[q]] + graphs[k].paths[p2][clusters[q]]) / graphs[k].paths[p][p2]
                ...
                if cluster_metric == -1 or cluster_metric == 2:
                    for method in range(len(methods)):
                        distance_cluster[:,:,method] += np.array([[parallel_dist_order(graphs[k], method, (i for i in clusters_idx_k[c]), q, 1)
                            for c in range(len(clusters_idx_k))] for q in clusters])
                    distance_cluster[:,:,-1] += np.array([[sum([graphs[k].paths[clusters[p]][q] + sum([graphs[k].paths[p2][q] for p2 in clusters_idx_k[c] if p2 > q]) for q in clusters_idx_k[c]
                                                                ]) / np.sum([graphs[k].paths[p][p2]
                            for q in clusters_idx_k[c] for p2 in clusters_idx_k[c] if p2 > q]) for c in range(len(clusters_idx_k))] for p in range(len(clusters))])
                ...
                count = 1
                print("Beginning EC")
                theta_k = [theta[q] + la_C_copy[k][clusters[q]] + np.sum(old_w[-1][idx_update:] * distance_cluster[q,find_clust(clusters[q],clusters_idx_k),:]) for q in range(len(clusters))]
                begin_EC = time.time()
...
if __name__ == "__main__":
    ...
    w, la_p, la_C, counter, conv_count, centers = learn(clustering_list, features_list, T, C, max_it,
                                                        ...
                                                        cpu_nb_slaves=cpu_nb_slaves, regu=regu, graphs=graphs, cluster_metric=cluster_metric)
    ...
    for set_i in range(len(clustering_list)):
        distances_list = classify.to_centers_distance_matrix(data[set_i], np.concatenate((data[set_i],data_val[set_i],
                                                            data_test[set_i]),axis=0),w, nn_numbers,centers[set_i],
                                                             clustering_list[set_i],order, False,[],
                                                             cluster_metric=cluster_metric,graph_train=[adjacency,
                                                            clustering_list[set_i] + clustering_list_val[set_i] +
                                                            clustering_list_test[set_i],clustering_list[set_i],proba])
        results_train, results_val, conf = classify.classify(data[set_i], data_val[set_i],clustering_list[set_i],
                            ...
                            centers[set_i], [0, len(clustering_list_val[set_i])], order, distances_list=distances_list)
