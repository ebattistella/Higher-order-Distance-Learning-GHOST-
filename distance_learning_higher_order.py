...
def convergence(old_w, conv_count, C, file):
    ...
    print(max([std_l[i] - crit_l[i] for i in range(len(l))]), file=file)
...
def learn(clustering_list, features_list, T, C, max_it, alpha=1., beta=1., speed=1., tau=0.5,
          update_name="id", nn_numbers=5, order=5, cpu_nb_slaves=20, regu="lasso", init=[], graphs=[], cluster_metric=0,
        ...
        for k in range(K):
            ...
            for q in range(len(clustering_list[k])):
                ...
                if cluster_metric > 0:  # If higher-order involved
                    ...
                    aux += np.array([sum(pool.starmap(parallel_dist_order,
                                                      ((graphs[k], method, combination, q,
                                                        1 / binom(
                                                            len(clusters_idx[k][i_clust]) - 2 + current_order - 1 - 1,
                                                            current_order - 1)) for
                                                       current_order in
                                                       range(2, order + 1) for combination in
                                                       combinations_with_replacement(clusters_idx[k][i_clust],
                                                                                     current_order - 1)))) for method in
                                     range(len(methods))] + [sum([
    ...
    while counter < max_it and conv_count < 3:
        ...
        for k in range(K):
            ...
            for i_clust in np.unique(clusters):
                ...
                sum_dist = np.sum(distance_i, axis=1)
                ...
                if cluster_metric != 0:
                    ...
                    sum_dist += distance_higher
                q_i = np.argmin(sum_dist)
                loss += sum_dist[q_i]
                # assign each point of the cluster to the exemplar
                for l in cluster_i:
                    x_k[l, cluster_i[q_i]] = 1
            print("iteration:", counter, "loss", loss, file=file)
            x[k] = x_k
        for i in range(T):
            ...
            for k in range(K):
                ...
                for p in range(len(clusters)):
                    ...
                    for q in range(len(clusters)):
                        ...
                        la_p[k][p, q] += compo
                        la_p[k][:, q] = np.round(la_p[k][:, q] - aux, decimals=12)
                        la_C[k][q] = np.round(la_C[k][q] - aux, decimals=12)
                ...
                for i_clust in np.unique(clustering_list[k]):
                    ...
                    for q in range(n_i):
                        ...
                        aux = float(compo / (n + 1))
                        la_C[k][clusters[cluster_i[q]]] = np.round(la_C[k][clusters[cluster_i[q]]] - (aux - compo))
                        la_p[k][:, clusters[cluster_i[q]]] = np.round(la_p[k][:, clusters[cluster_i[q]]] - aux)
        ...
        # Convergence check
        conv_count = convergence(old_w, conv_count, C, file=file)
        sys.stdout.flush()
    ...
    return w, la_p, la_C, counter, conv_count, centers, graphs
...
if __name__ == "__main__":
    ...
    print(d, r, c, n, proba, cluster_metric, output_path)
    alpha = 1.0
    beta = 1.0
    speed = 1
    tau = .5
    ...
    seed = 50
    np.random.seed(seed)
    rand_mean = [(10 * i + 1) * np.array([np.random.randint(-50, 50) for j in range(d)]) for i in range(c)]
    rand_cov = [np.diag([np.random.randint(0, 200) * (10 * i + 1) if j < d - r else np.random.randint(0, 200)
                                            for j in range(d)]) for i in range(c)]
    data = np.concatenate(
        [np.random.multivariate_normal((10 * i + 1) * np.array([np.random.randint(-50, 50) for j in range(d)]),
                                       cov + np.diag(
                                           [np.random.randint(0, 200) * (
                                                       10 * i + 1) if j < d - r else np.random.randint(0, 200)
                                            for j in range(d)]),
                                       n + (10 * i + 1)) for i in range(c)])
    features_list = [np.array(data)]
    clustering_list = [
        [k for k in range(c) for _ in range(n + (10 * k + 1))]]
    aux_mean = np.mean(np.concatenate(features_list, axis=0), axis=0)
    aux_std = np.std(np.concatenate(features_list, axis=0), axis=0)
    data = features_list.copy()
    ...
    if has_vector:
        dists_names = ['euclidean', 'minkowski', 'cityblock', 'cosine', 'correlation', 'hamming', 'jaccard',
                       'chebyshev', 'matching', 'yule', 'braycurtis', 'dice', 'russellrao']
        dists_vectors = lambda x, y, i: pdist(np.array([x, y]), i)
        features_list[0] = np.concatenate(
            (features_list[0], [[np.concatenate([dists_vectors(data[0][p, :], data[0][q, :], dist_i) for dist_i in
                                                 dists_names]) for p in range(np.shape(data[0])[0])] for q in
                                range(np.shape(data[0])[0])]), axis=2)

    output_path = output_path + "_".join(
        [str(alpha), str(beta), str(speed), str(tau), update_name, str(nn_numbers), str(order), str(has_vector), regu,
         str(cpu_nb_slaves)] + sys.argv[1:-1]) + "/"
    ...
    for i in range(c):
        for k in range(c):
            if i > k:
                ...
                adj = np.array(
                    np.array(np.random.rand(n + 10 * i + 1, n + 10 * k + 1) - proba >= 0, int) == int(i == k), int)
                aux_adj[i].append(adj)
    adjacency = np.block(aux_adj)
    graphs = [graph.Graph.from_adjacency(adjacency)]
    if not os.path.exists(output_path + "w.npy"):
        before_learning = time.time()
        with open(output_path + "convergence.txt", "w") as cv_file:
            w, la_p, la_C, counter, conv_count, centers, graphs = learn(clustering_list, features_list, T, C, max_it,
                                                                        alpha=alpha, beta=beta, speed=speed, tau=tau,
                                                                        update_name=update_name, nn_numbers=nn_numbers,
                                                                        order=order,
                                                                        cpu_nb_slaves=cpu_nb_slaves, regu=regu,
                                                                        graphs=graphs, cluster_metric=cluster_metric,
                                                                        file=cv_file)
        learned = time.time()
        print("learning time:", learned - before_learning)
        print("counter", counter, "conv count:", conv_count)
        print("Centers:", centers)
        print("Weights:", w)
        np.save(output_path + "w.npy", w)
        np.save(output_path + "centers.npy", centers)
    else:
        centers = np.load(output_path + "centers.npy")
        w = np.load(output_path + "w.npy")
        max_path = 10 ** 1 * np.max(features_list[0], axis=None)
        for g in graphs:
            # Pre-computation of the path lengths
            g.path_finding(max_path)
    ...
    data_val = np.concatenate(
        [np.random.multivariate_normal((10 * i + 1) * np.array([np.random.randint(-30, 30) for j in range(d)]),
                                       cov_val + np.diag(
                                           [np.random.randint(0, 200) * (10 * i + 1) if j < d - r else np.random.randint(0, 200)
                                            for j in range(d)]),
                                       n + (10 * i + 1)) for i in range(c)])
    ...
    if has_vector:
        features_list_val[0] = np.concatenate((features_list_val[0], [
            [np.concatenate([dists_vectors(data_val[0][p, :], data_val[0][q, :], dist_i) for dist_i in
                             dists_names]) for p in range(np.shape(data_val[0])[0])] for q in
            range(np.shape(data_val[0])[0])]), axis=2)
    # Testing Set
    mean_test = np.array([50 for i in range(d)])
    cov_test = np.diag([30 if i < d - r else 10000 for i in range(d)])

    data_test = np.concatenate(
        [np.random.multivariate_normal(0.5 * (10 * i + 1) * np.array([np.random.randint(-50, 50) for j in range(d)]),
                                       cov_test + np.diag(
    ...
    if has_vector:
        features_list_test[0] = np.concatenate((features_list_test[0], [
            [np.concatenate([dists_vectors(data_test[0][p, :], data_test[0][q, :], dist_i) for dist_i in
                             dists_names]) for p in range(np.shape(data_test[0])[0])] for q in
            range(np.shape(data_test[0])[0])]), axis=2)
    with open(output_path + "results.txt", "w") as results_file:
        # In case one would add other training, validation, testing sets
        for set_i in range(len(clustering_list)):
            distances_list = classify.to_centers_distance_matrix(data[set_i],
                                                                 np.concatenate((data[set_i], data_val[set_i],
                                                                                 data_test[set_i]), axis=0), w,
                                                                 centers[set_i],
                                                                 clustering_list[set_i], order, output_path + "dist",
                                                                 graph_train=[graphs[set_i],
                                                                              clustering_list[set_i] +
                                                                              clustering_list_val[set_i] +
                                                                              clustering_list_test[set_i], proba],
                                                                 cluster_metric=cluster_metric, second_order_dist=dists)
            results_train, results_val, conf = classify.classify(0, len(clustering_list[set_i]), clustering_list[set_i],
                                                                 clustering_list_val[set_i],
                                                                 centers[set_i], distances_list=distances_list)

            _, results_test, conf_test = classify.classify(0, len(clustering_list[set_i]) + len(
                clustering_list_val[set_i]), clustering_list[set_i],
                                                           clustering_list_test[set_i], centers[set_i],
                                                           distances_list=distances_list)
            print(results_train, results_val, results_test, file=results_file)
    print(output_path + "results.txt")
