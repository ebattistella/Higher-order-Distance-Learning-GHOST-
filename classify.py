...
def parallel_dist_aux(p,q,m, data1, data2, has_vector):
    ...
    if p != q:
        return [dists[i](data1[p,i], data2[q,i]) for i in range(m)] + [dist(data1[p,:], data2[q,:]) for dist in dists_vectors]
    else:
...
def to_centers_distance_matrix(distance, data_train, data_test, w, nn_numbers, centers, clusters, order, has_vectors,
    ...
    if not os.path.exists(path + "train.npy"):
        ...
        aux_distance_train = np.array(pool.starmap(parallel_dist_aux,
                                                   ((p, q, np.shape(data_train)[1], data_train, data_train, has_vectors) for p in
                                                    range(n) for q in
    ...
    if not os.path.exists(path + "train_test.npy"):
        ...
        aux_distance_train_test = np.array(pool.starmap(parallel_dist_aux,
                                                        ((p, q, np.shape(data_test)[1], data_test, data_train, has_vectors) for
                                                         p in
                                                         range(n_2) for q in
                                                         range(n))), dtype=object).reshape(
        ...
        for idx_train in range(len(centers)):
            ...
            if w[-1] != 0:
                ...
                if cluster_metric > 0:
                    ...
                    if not os.path.exists(path + "p" + str(patient)+"center"+ str(idx_train)+"graph" + ".npy"):
                        ...
                        aux_para = np.load(path + "p" + str(patient) + "center" + str(idx_train) + "graph" + ".npy", allow_pickle = True)
                    aux_para = aux_mult * aux_para
                if cluster_metric == -1 or cluster_metric == 2:
                    aux_para += sum([g.paths[train_size][q] + sum([g.paths[p2][q] for p2 in cluster_k if p2 > q]) for q in cluster_k
                                                                ]) / np.sum([g.paths[p][p2]
            ...
            for method_i in range(len(methods_func)):
                if w[-1 - 1 - method_i] != 0:
                    aux_para = 0
                    if cluster_metric > 0 :
                        if not os.path.exists(path + "p" + str(patient) + "center" + str(idx_train) + "method" + str(method_i) + ".npy"):
                            aux_para += sum(pool.starmap(parallel_dist_order,
                                         ((g, method_i, list(combination) + [idx_train],
                                           train_size, 1/binom(len(cluster_k)-2+current_order-1-2, current_order-2))
                                          for current_order in range(2, order) for combination in
                                          combinations_with_replacement(cluster_k, current_order - 2))))
                            np.save(path + "p" + str(patient) + "center" + str(idx_train) + "method" + str(method_i), aux_para)
                        else:
                            aux_para += np.load(path + "p" + str(patient) + "center" + str(idx_train) + "method" + str(method_i) + ".npy",
                                           allow_pickle=True)
                    if cluster_metric == -1 or cluster_metric == 2:
